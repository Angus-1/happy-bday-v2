 export const signatures = [   
    {
        name: 'Angle📐',
        message: "Happy birthday Gamer!!",
        message2: "have a happy mf day! enjoy this cat site" ,
        link: 'https://cat-bounce.com/',
        image: 'monkey.gif'
    },
    {
        name: 'Daniel',
        message: " Happy birthday Charlie barbie,",
        message2: "You still owe me those mf Idaho potatoes from last month or sth, WHERE IS MY DAMN POTATOES" ,
        link: '',
        image: 'potato.png'
    },
    {
        name: 'Jacob C',
        message: "",
        message2: "" ,
        link: '',
        image: 'chalrie.png'
    },
    {
        name: 'Bev',
        message: "Happy Birfday dude!!!! Cheers to 22",
        message2: "🐒" ,
        link: '',
        image: 'smile.png'
    },
    {
        name: 'Deanna',
        message: "Happy Birthday stinky!",
        message2: "" ,
        link: '',
        image: 'joker.png'
    },
    {
        name: 'Christos',
        message: "happy birthday.",
        message2: "" ,
        link: '',
        image: 'frog.png'
    },
    {
        name: 'Yao',
        message: "happy birthday charlie. take it easy and hope you have a great one        ",
        message2: "" ,
        link: '',
        image: 'anime.png'
    },
    {
        name: 'Alan',
        message: "Happy Birthday Monke Man",
        message2: "I hope you have a miraculously monkey birthday enjoy this goated MonkE cranking 90s" ,
        link: '',
        image: 'monkey2.gif'
    },
    {
        name: 'Jacob N',
        message: "Happy 22nd Birthday Charlie. You found a shiny monke!        ",
        message2: "" ,
        link: '',
        image: 'chimchar.png'
    },
    {
        name: 'Aaron',
        message: "",
        message2: "" ,
        link: '',
        image: 'coding.gif'
    },
    {
        name: 'Steph',
        message: "Happy birthday Monkey Man!! ",
        message2: "Being your friend has been super fun and I'm glad we've shared these good times together! Have fun this weekend and drink plenty of mint mojitos for me. kys" ,
        link: '',
        image: 'snapchat.png'
    },
    {
        name: 'Justin',
        message: "Happy Birthday Chewie,",
        message2: "I hope you shrink a few inches this year because it's not fair for the rest of us." ,
        link: '',
        image: 'height.png'
    },
    {
        name: 'EthanThatOneKid',
        message: "YOOOOOOOOOOOOOOOOOOOOOOO!",
        message2: "Happy birthday Charlie!! Welcome to level 22!",
        link: '',
        image: "twrp.jpg"
    },
    {
        name: 'Karni',
        message: "Dear Charles Truman Taylor III,",
        message2: "Rember my party and we pass lemon? I laff. Happ Birth.",
        link: '',
        image: "tracksuit.png"
    },
]